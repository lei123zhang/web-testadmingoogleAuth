  ...
  <div>
     <Card>
      ...
      <Row class="userDetails">
        ...
        <span>用户名：<i>{{ userInfo.memberUsername }}</i></span>
        <span>真实姓名：<i>{{ userInfo.memberRealName }}</i></span>
        <span>到账数量：<i>{{ userInfo.arrivedAmount }}</i></span>
        <span>提现地址：<i>{{ userInfo.address }}</i></span>
        
        <br>
        <span>提现币种：<i>{{ userInfo.unit }}</i></span>
      ...
      <Modal 
        ...
        v-model="ifPassCoin"
        @on-ok="confrimPass">
        <div>请输入流水号：<p><Input type="textarea" v-model="transactionNum"></Input></p></div>
      </Modal>
...
import { auditPass, auditNoPass, withdrawManage, perTradeAll, passCoinByOne } from '@/service/getData';
      ...
      return {
				...
				},
        {
          ...
          render: (h, obj) => {
						...
						let typeTxt = null;
						let arr = ['充值', '提现', '转账', '币币交易', '法币买入', '法币卖出', 
