			...
			<Card>
				...
				<Modal v-model="showEditModal" 
					...
					<ul>
						...
						<li><span>承接人：</span>{{`${modelInner.associateName}(${modelInner.associateUsername})`  }}</li>
...
export default {
  ...
  methods: {
    ...
    searchByFilter() {
			...
			let subObj = { pageNo: 1, pageSize: 10};
			 if(this.filterSearch.success=== -1) {
				this.filterSearch.success=null;
				this.filterSearch.auditing=true;
			 }else if(this.filterSearch.success=== '') {
				this.filterSearch.auditing=false;
			 }
      Object.assign(subObj, this.filterSearch);
			this.refreshPage(subObj);
			if(this.filterSearch.success===null) {
				this.filterSearch.success=-1;
			}
		},
    refreshPageManual() {
			this.currentPageIdx = 1;
