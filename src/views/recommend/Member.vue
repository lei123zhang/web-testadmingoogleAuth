    ...
    <Card>
      <p slot="title">
        ...
        <Button type="primary" size="small" @click="refreshPageManual">
          ...
          <div class="poptip">
            <Poptip trigger="hover" content="请输入被推荐人呢称、邮箱或手机号搜索" placement="bottom-start">
              <Input placeholder="请输入被推荐人呢称、邮箱或手机号搜索" 
...
export default {
      ...
      columns_first: [
				...
				},
        {
					...
					render: (h, obj) => {
						...
						else return `${reward[0][1]} ${reward[0][0]}`;
          ...
          title: "操作",
					...
					render: (h, obj) => {
						return h('Button',{
							...
							on: {
								click: () => {
									this.$router.push( {path: '/recommend/member/commissiondetail', query:{ memberId: obj.row.id }})
								}
							}
						}, '查看')
					}
  ...
  methods: {
		searchByFilter() {
			this.aLink = `${BASICURL}admin/promotion/member/out-excel?${qs.stringify(this.filterSearch)}`;
