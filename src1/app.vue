				...
				<div class="businessCheckMask" v-show="memberCheckMask">
					<div class="wrapper">
						...
						<Row class="businessInfo">
							...
							<Col span="8">身份证号码：{{ !memberCheckImgInfo.member ? '' : memberCheckImgInfo.member.member.idNumber }}</Col>
						</Row>
						<Row class="img">
  ...
  created() {
	...
	methods: {
		businessNoPass() {
			...
			businessAudit(userInfo.member.id, { status: 3, detail: this.rejectBusinessReason })
			.then(res => {
				if(!res.code) {
